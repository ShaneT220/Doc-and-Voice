...
def getEmbeddingDiscrepency(embedding, text):
    context = index.query(
        vector = embedding,
        top_k = 10,
...
def getEmbeddingSimilarity(embedding, text):
    context = index.query(
        vector = embedding,
        top_k = 10,
        include_metadata = True
...
def getAudioSummary(text):
    prompt =  f"""Summarize the following minute worth of audio in bullet points. Gear the summary towards informing attorneys and keep it as concise and clear as possible.
